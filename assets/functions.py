...
def save_pkl(data, file_name):
		...
		Args:
				...
				file_name (str): The name of the pickle file (excluding the '.pkl' extension).
...
def gather_links(url):
  ...
  Args:
    ...
    files_list (list): A list to store the gathered file links.
  ...
  for link in soup.find_all("a"):
    ...
    if full_link.startswith(b3.base_cvm) and full_link not in b3.visited_subfolders:
      if full_link.endswith((".csv", ".zip", ".txt")):
        b3.files_list.append(full_link)
      elif full_link.endswith("/"):
        gather_links(full_link)
  return b3.files_list
...
def get_metadados(files_list):
    ...
    Args:
        files_list (list): A list of file links containing files with metadata.
