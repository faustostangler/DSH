...
def get_company(i, driver, wait): # old def, is it usefull, is another one better??
  ...
  try:
    ...
    for word, replacement in list_dict.items():
      listagem = main.clean_text(listagem.replace(word, replacement))
  except Exception as e:
    listagem = ''
  ...
  try:
    ...
    company_name = main.clean_text(wText(xpath, wait))
  except Exception as e:
    company_name = ''
  ...
  try:
    ...
    pregao = main.clean_text(wText(xpath, wait))
  except Exception as e:
    pregao = company_name
  ...
  try:
    ...
    segmento = main.clean_text(setor.split(' / ')[2].strip())
    subsetor = main.clean_text(setor.split(' / ')[1].strip())
    setor = main.clean_text(setor.split(' / ')[0].strip())
  except Exception as e:
    setor = ''
    subsetor = ''
    segmento = ''
  ...
  try:
    ...
    escriturador = main.clean_text(' '.join(pd.Series(escriturador).drop_duplicates().tolist()))
  except Exception as e:
    escriturador = ''
  ...
  for inner_html in raw_code:
    ...
    for card in cards:
      try:
        ...
        ticker = main.clean_text(card.find('h5', class_='card-title2').text)
        company_name = main.clean_text(card.find('p', class_='card-title').text)
        pregao = main.clean_text(card.find('p', class_='card-text').text)
        listagem = main.clean_text(card.find('p', class_='card-nome').text)
        if listagem:
            for abbr, full_name in b3.abbreviations_dict.items():
                new_listagem = main.clean_text(listagem.replace(abbr, full_name))
...
def nsd_get_nsd(nsd):
  ...
  company = main.clean_text(company)
  company = main.word_to_remove(company)
  ...
  data = [main.clean_text(item) if item not in [company, envio, url] else item for item in data]
...
def nsd_get_nsd_content():
    try:
        ...
        for i, n in enumerate(range(start, end)):
            ...
            progress = main.remaining_time(start_time, end-start, i)
            try:
...
def stk_get_composicao_acionaria():
    ...
    try:
        ...
        for j, (i, row) in enumerate(filtered_nsd.iterrows()):
            ...
            print(main.remaining_time(start_time, len(filtered_nsd), j), i, data[0], data[1], data[2], data[3], data[4], data[5])
...
def item_to_list(item):
    ...
    if type(item) is list:
        item = [main.clean_text(word) for word in item]
        item = '|'.join(item)
    ...
    try:
        if line != '':
            ...
            if startswith != '':
                ...
                item = item.split('|')[0]
                startswith_mask = (df[line].apply(lambda word: main.clean_text(word)).str[:len(item)] == item).replace({np.nan: False})

            # startswith_not
            if startswith_not != '':
                item = item_to_list(startswith_not)
                item = item.split('|')[0]
                startswith_not_mask = ~(df[line].apply(lambda word: main.clean_text(word)).str[:len(item)] == item).replace({np.nan: False})

            # endswith
            if endswith != '':
                item = item_to_list(endswith)
                item = item.split('|')[0]
                endswith_mask = (df[line].apply(lambda word: main.clean_text(word)).str[-len(item):] == item).replace({np.nan: False})

            # endswith_not
            if endswith_not != '':
                item = item_to_list(endswith_not)
                item = item.split('|')[0]
                endswith_not_mask = ~(df[line].apply(lambda word: main.clean_text(word)).str[-len(item):] == item).replace({np.nan: False})

            # contains
            if contains != '':
                item = item_to_list(contains)
                contains_mask = df[line].apply(lambda word: main.clean_text(word)).str.contains(item, case=False).replace({np.nan: False})

            # contains_not
            if contains_not != '':
                item = item_to_list(contains_not)
                contains_not_mask = ~df[line].apply(lambda word: main.clean_text(word)).str.contains(item, case=False).replace({np.nan: False})
...
def filter_conditions(df, line, conditions):
    ...
    for condition, value in conditions.items():
        ...
        if operation == "exact":
            ...
            mask &= (df[line].apply(lambda word: main.clean_text(word)).str[:len(value)] == value).replace({np.nan: False})
...
def fundamentalist_dre(df, group):
    try:
      ...
      for _, row in df.iterrows():
          key = main.clean_text(row['Conta'] + ' - ' + row['Descrição']).split('  ')
...
def create_demofile():
    ...
    try:
        ...
        for i, year in enumerate(years):
            print(main.remaining_time(start_time, len(years), i))
            dataframe = main.load_pkl(f'{b3.app_folder}/dataframe_{year}')
...
def process_year_data(year, df_merged, where):
    ...
    try:
        ...
        return year, main.load_pkl(f'{b3.app_folder}/temp_math_{where}_{year}')
    except Exception as e:
        try:
...
# Get all unique companies across all years
    ...
    try:
        ...
        #   if company == 'ALPARGATAS SA':
            print(main.remaining_time(start_time, len(all_companies), i))
...
def math_from_cvm(cvm_local):
    ...
    for year in years:
        try:
            math_local[year] = main.load_pkl(f'{b3.app_folder}/math_web_{year}')
        except Exception as e:
...
def b3_get_company_full_info(data, full_company_info):
    ...
    for item in reversed(data):
        ...
        if match:
            ...
            full_company_info["setor"] = main.clean_text(setor)
            full_company_info["subsetor"] = main.clean_text(subsetor)
            full_company_info["segmento"] = main.clean_text(segmento)
            data.remove(item)
            break
        ...
        try:
            ...
            for i in range(len(company_names)):
                company_list_from_web.append({
                    'COMPANHIA': main.clean_text(company_names[i]),
...
def get_b3_companies_from_site(driver, wait, url):
    ...
    try:
        ...
        for tp in range(total_pages):
            print(main.remaining_time(start_time, total_pages, tp))
...
def b3_grab_from_web_OLD(driver, wait, url): # OLD DEF, or still usefull. b3_get_companies is it better??
    ...
    try:
        ...
        for i, page in enumerate(range(0, pages+1)):
            ...
            time.sleep(0.5)
            value = f'page {page+1}'
            print(main.remaining_time(start_time, pages+1, i), value)
        ...
        for i, (index, row) in enumerate(b3_companies_tickers.iterrows()):
            ...
            if new_keyword not in b3_companies_local_keywords:
                ...
                pass
            print(main.remaining_time(start_time, len(b3_companies_tickers), i), counter, size-counter, new_keyword)
...
def b3_get_company_info(row, driver, wait):
    ...
    for attempt in range(max_retries):
        try:
            ...
            for card in cards:
                ticker = main.clean_text(card.find_element(By.XPATH, ".//h5[@class='card-title2']").text)
                company_name = main.clean_text(card.find_element(By.XPATH, ".//p[@class='card-title']").text)
    ...
    for attempt in range(max_retries):
        try:
            ...
            stock_holders['Nome'] = stock_holders['Nome'].apply(main.clean_text) # Apply main.clean_text only to the 'Nome' column in the DataFrame
...
def b3_get_company_extra_cnpj_info(response):
    ...
    for p in p_elements:
        ...
        if 'Razão Social:' in text_content:
            ...
            if razao_social:
                info['Razão Social'] = main.clean_text(razao_social[0])
        
        if 'Nome Fantasia:' in text_content:
            nome_fantasia = p.xpath('.//b[@class="copy"]/text()')
            if nome_fantasia:
                info['Nome Fantasia'] = main.clean_text(nome_fantasia[0])
...
def b3_get_companies(url):
    ...
    # b3_companies_tickers = main.load_pkl('temp_b3_companies_tickers')
    ...
    try:
        ...
        for i, (index, row) in enumerate(updated.iterrows()):
            ...
            print(main.remaining_time(start_time, size, i), row['ticker'], row['company_name'])
    ...
    for idx, opt in enumerate(select.options):
        ...
        for attempt in range(max_retries):
            try:
                ...
                setor = main.clean_text(option_value)
...
def get_segmentos(driver, wait, option_value):
    ...
    for idx, _ in enumerate(rows):
        ...
        subsetor = main.clean_text(subsetor)
        ...
        for seg_idx, _ in enumerate(segmentos):
            ...
            segmento = main.clean_text(item.text)
            item.click()
            ...
            for company in companies:
                company_row = [main.clean_text(setor), subsetor, segmento, link]
                company_row.extend([company[key] for key in ['COMPANHIA', 'PREGAO', 'TICK', 'LISTAGEM']])
                setorial.append(company_row)

            print(f'{main.clean_text(setor)}: {subsetor}: {segmento}: {len(companies)}:')
...
def apply_intel_rules(df, rules):
    ...
    try:
        ...
        for r, (line, conditions) in enumerate(rules):
            ...
            print(main.remaining_time(start_time, len(rules), r), line, 'matching rows', len(matching_rows))
...
def prepare_b3_cvm(b3_cvm):
    ...
    try:
        ...
        for k, (key, df) in enumerate(b3_cvm.items()):
            print(key, main.remaining_time(start_time_b3, len(b3_cvm), k))
...
def compose_intel(acoes, intel_b3):
    ...
    for i, (setor, df) in enumerate(intel_b3.items()):
        ...
        print(main.remaining_time(start_time, len(intel_b3), i), setor)
    return intelacoes
        ...
        for i, (setor, df) in enumerate(intelacoes.items()):
            ...
            for j, (key, sheet) in enumerate(sheets):
                ...
                if j% (b3.bin_size * 2) == 0:
                    print(setor, company, main.remaining_time(start_time_2, len(sheets), j))
            ...
            print(setor, main.remaining_time(start_time, len(intelacoes), i))
    except Exception as e:
        pass
    return fund
...
def get_yahoo_quotes(ticker, start_date, end_date=pd.Timestamp.today().strftime('%Y-%m-%d'), country='brazil', interval='1d', events='history', includeAdjustedClose=True):
    ...
    for i, tick in enumerate(ticker):
        ...
        try:
            ...
            quotes[tick] = df
            # print(run.main.remaining_time(start_time, len(ticker), i), tick)
        except Exception as e:
            pass
        ...
        for j, (index, row) in enumerate(df_tickers.iterrows()):
            ...
            try:
                ...
                if start_date < pd.Timestamp.today().strftime('%Y-%m-%d'):
                    ...
                    print(main.remaining_time(start_time, len(fund), i), setor, main.remaining_time(start_time_2, len(df_tickers), j), pregao, ', '.join(ticker))
            except Exception as e:
                pass
...
def integrate_yahoo_quotes(fund):
    ...
    try:
        ...
        quotes = main.load_pkl(f'{b3.app_folder}/quotes')
    except Exception as e:
...
def merge_quotes(fund, quotes):
    ...
    try:
        ...
        for i, (setor, df) in enumerate(fund.items()):
            ...
            print(setor, main.remaining_time(start_time, len(fund), i))
    except Exception as e:
        pass
        ...
        for i, (setor, df) in enumerate(df_preplot.items()):
            ...
            for i2, company in enumerate(companies):
                ...
                try:
                    ...
                    pass
            print(setor, main.remaining_time(start_time, len(df_preplot), i))
    except Exception as e:
        pass
